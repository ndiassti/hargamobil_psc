...
# =========================================
# 1️⃣ Load model lokal (tanpa download)
# =========================================
MODEL_PATH = "model_super_compressed.pkl.xz"

if not os.path.exists(MODEL_PATH):
    st.sidebar.error(f"❌ File model {MODEL_PATH} tidak ditemukan. Pastikan sudah diunggah ke repo.")
    st.stop()
else:
    st.sidebar.info("📦 Memuat model terkompresi dari file lokal...")

# =========================================
# 2️⃣ Set API Key biar Gemini jalan
# =========================================
...
# Otomatis deteksi model tanpa input manual
if os.path.exists("model_super_compressed.pkl.xz"):
    model_path = "model_super_compressed.pkl.xz"
    st.sidebar.write("📦 Menggunakan model terkompresi: model_super_compressed.pkl.xz")
elif os.path.exists("model.pkl"):
    model_path = "model.pkl"
    st.sidebar.write("📦 Menggunakan model standar: model.pkl")
else:
    st.sidebar.error("❌ Tidak ditemukan file model (model_super_compressed.pkl.xz atau model.pkl).")
    st.stop()
